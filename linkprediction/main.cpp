...
private:
	...
	void bfgs()
	{
		...
		for (int epoch = 0; epoch<nepoch; epoch++)
		{
			...
			FILE* f2 = fopen(("relation2vec_n150." + version).c_str(), "w");
			FILE* f3 = fopen(("entity2vec_n150." + version).c_str(), "w");
			for (int i = 0; i<relation_num; i++)
			{
				for (int ii = 0; ii<n; ii++)
...
void prepare()
{
	...
	FILE* f_kb = fopen("train1.txt", "r");
	while (fscanf(f_kb, "%s", buf) == 1)
	{
		//train1.txt 为head relation tail 顺序，故把s2改为s3
		string s1 = buf;
		fscanf(f_kb, "%s", buf);
		string s3 = buf;
		fscanf(f_kb, "%s", buf);
